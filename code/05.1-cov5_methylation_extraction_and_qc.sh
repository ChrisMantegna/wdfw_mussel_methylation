...
METHYL_DIR="${METHYL_DIR:-$SCRIPT_DIR/../output/methylation_extraction}"
REPORT_DIR="${REPORT_DIR:-$METHYL_DIR/reports}"
FILTER_DIR="${FILTER_DIR:-$METHYL_DIR/filtered_cov}"
LOG_DIR="${LOG_DIR:-$METHYL_DIR/logs}"
...
# Input BAMs
# Prefer deduplicated BAMs in read-pair order.
...
for bam in \
  ...
  "$DEDUP_DIR"/*_name_sorted.bam \
  "$DEDUP_DIR"/*.name_sorted.bam
do
  [ -f "$bam" ] || continue
...
if [ ${#BAMS_RAW[@]} -eq 0 ]; then
  echo "ERROR: No suitable paired-end BAM files found in $DEDUP_DIR" >&2
  echo "Expected deduplicated BAMs or name-sorted BAMs, not coordinate-sorted BAMs." >&2
  exit 1
fi
